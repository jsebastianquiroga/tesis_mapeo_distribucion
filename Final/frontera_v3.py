...
#         M_vectors = np.random.randn(C, len(np.unique(self.y))) # use len() instead of np.nunique()
#         R = self.X_1 @ M_vectors # replace A with self.X_1
#         S = np.where(R>0, 1, 0)

#         # convert binary vector to integer
#         binary_column = 2**np.arange(len(np.unique(self.y))).reshape(-1,  1)

#         # convert each band into a single integer
#         S = np.hstack([M @ binary_column for M in S])

#         # append integer vectors to matrix A
#         self.lsh_A = np.column_stack((self.X_1, S)) # replace lsh_A with self.lsh_A

#         # calculate the dimensions of the centroid
#         self.lsh_A_d = np.mean(self.lsh_A, axis=0) # replace lsh_A with self.lsh_A

    def lsh(self):
        F, C = self.X_1.shape
        self.num_hash_vectors = len(self.y_1.unique())
        
        # Compute signature matrix
        M_vectors = np.random.randn(C, self.num_hash_vectors)
        R = X @ M_vectors
        S = np.where(R > 0, 1, 0)

        # Convert each band into a single integer
        binary_column = 2 ** np.arange(self.num_hash_vectors).reshape(-1, 1)
        S = np.hstack([M @ binary_column for M in S])

        # Stack the LSH data with the original data
        self.lsh_data = np.column_stack((self.X_1, S))
        
        # Group the data based on the LSH groups
        lsh_groups = defaultdict(list)
        for i, row in enumerate(self.lsh_data):
            group_key = tuple(row[-self.lsh_data.shape[1]//2:])
            lsh_groups[group_key].append(i)
        
        # Compute the centroids of each LSH group
        centroids = []
        for group_key in lsh_groups:
            group_indices = lsh_groups[group_key]
            group_data = self.lsh_data[group_indices, :-self.num_hash_vectors]
